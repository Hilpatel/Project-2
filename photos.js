...
function addFavourite(imgSrc) {
    var fav = document.querySelectorAll("#gallery img");
    var favCount = fav.length;
    if (favCount < 5) {
        console.log("we have come up to here!!!");
        var newDiv = document.createElement("div");
        var newImage = document.createElement("img");
        var removeButton = document.createElement("button");
        removeButton.classList.add("rmvButton");
        newDiv.classList.add("newImgDiv");
        
        removeButton.innerText = 'Remove';

        newImage.src = imgSrc;
        newImage.className = "favourite_img";
        newDiv.appendChild(newImage);
        newDiv.appendChild(removeButton);
        document.getElementById("gallery").appendChild(newDiv);
        clickFaveImg(newDiv, removeButton);
...
/* find who is clicked */
function clickFaveImg(newDiv, removeButton) {
    newDiv.addEventListener('click', function () {
        removeButton.style.display = "inline-flex";
    });

    removeButton.addEventListener('click', function (event) {
        event.target.parentNode.parentNode.removeChild(newDiv);
    });
